...
| Bridge Controller | `0.3.12` 自动化候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | `0.3.12` 为远端工具结果和审计增加显式根身份，并对尚未实现的本地根访问失败关闭；候选 VSIX 安装与双平台实机待补测 |
...
`docs/acceptance/2026-07-23-release-0.3.12-root-identity-protocol.md`；上一支持基线见
