...
Executor 配套版本自动同步、官方新任务、当前 Shim、远端项目操作，以及 `0.3.61`
...
| Bridge Controller | `0.3.61` Windows x64 候选；`0.3.51` Linux Remote SSH 候选；`0.2.7` Windows 支持基线 | 同一扩展 ID；Linux/Windows 必须原生构建并以受控 stage 收集，禁止异平台启动器交叉构包 | `0.3.61` 已安装在 Windows x64；thread 和每轮上下文包含统一路由清单，MCP 家族以 `route-configured` 区分路由配置与具体工具可用性，无参数形状推断；真实远端 `pwd` 通过，性能分段与完整双平台门禁待补测 |
