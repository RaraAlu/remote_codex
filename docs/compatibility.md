...
| Bridge Controller | `0.3.14` 自动化候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | `0.3.14` 增加本地次级根与远程主根的统一只读工具路由、Controller 本地请求回传和原生 UI 投影；候选 VSIX 安装与双平台实机待补测 |
| Remote Executor | `0.2.9`，Linux x64 自动化候选 | Workspace 扩展；通过当前 Remote SSH 通道自动部署；ping 按所需能力集合验收，不按包版本或协议号门禁 | 搜索统一为大小写敏感的字面匹配；能力集合和协议形状未变，候选实机待补测 |
...
`docs/acceptance/2026-07-23-release-0.3.14-dual-read-routing.md`；上一支持基线见
