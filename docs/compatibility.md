...
| Bridge Controller | `0.3.8` Linux x64 候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | `0.3.8` 在 `0.3.6` 正常关闭基础上完成设置恢复、恢复驱动停止和停用空闲态；Linux Remote SSH 实机通过，Windows 实机待补测 |
...
`docs/acceptance/2026-07-23-release-0.3.8-settings-restore-idle.md`；上一支持基线见
