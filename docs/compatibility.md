...
| Bridge Controller | `0.3.20` 自动化候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | 双端写入和后台任务生命周期已通过自动化；候选 VSIX 安装与双平台实机待补测 |
...
`docs/acceptance/2026-07-24-release-0.3.20-background-tasks.md`；风险命名空间
