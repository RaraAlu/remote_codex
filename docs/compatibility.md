...
| Bridge Controller | `0.3.13` 自动化候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | `0.3.13` 增加本地次级根授权、撤销、诊断和限额只读执行器；双端工具路由尚未开放，候选 VSIX 安装与双平台实机待补测 |
...
`docs/acceptance/2026-07-23-release-0.3.13-local-root-authority.md`；上一支持基线见
