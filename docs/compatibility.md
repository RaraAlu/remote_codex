...
| Bridge Controller | `0.3.17` 自动化候选；`0.2.7` 支持基线 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | `0.3.17` 在有副作用请求的 transport 中断后通过新 socket 查询原幂等键；明确恢复终态或返回 `RESULT_UNKNOWN`，不会重放原命令；候选 VSIX 安装与双平台实机待补测 |
...
`docs/acceptance/2026-07-23-release-0.3.17-disconnect-recovery.md`；上一支持基线见
