...
| Bridge Controller | `0.2.0`，Windows x64、Linux x64 | 同一扩展 ID，分别发布 `win32-x64` 和 `linux-x64` VSIX | 两个目标包均已生成并检查内容 |
| Remote Executor | `0.2.0`，Linux x64 | Workspace 扩展；通过当前 Remote SSH 通道自动部署，不含 Codex 或凭据 | 构建和打包通过；真实窗口回环继续验收 |
