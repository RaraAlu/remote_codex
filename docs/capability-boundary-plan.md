...
| CLI-INTERVENE | 本地外部 Codex CLI 接入反代对话并介入写入 | 最终待实施 | Controller 已有本地 IPC、线程代理、远程主根与审计基础 | 没有稳定插件 API、对话代理、多客户端仲裁、外部 turn 写入或受控文件写入 |
...
10. 外部 Codex CLI 仅作为最后接入的本地客户端，不替代官方扩展内置 app-server，
    不成为运行时发现或回退来源；对话和项目写入必须经过 Controller 插件接口。
...
### 阶段 9：本地外部 Codex CLI 介入

目标：在前八个阶段全部闭环后，让本地运行的外部 Codex CLI 通过 Controller 插件接口
访问 Remote Bridge 反代后的对话，并按 thread 的 Codex 权限模式介入对话与远程项目
写入。远端不安装 Codex，系统 CLI 不参与官方 app-server 的发现、选择、透传或回退。
通常使用的 `full-access` 必须在已选目标端提供最大操作权限，不增加 Bridge 二次审批。

前置能力探查：

1. 重新生成并检查当时官方扩展内置 app-server 协议，确认 thread/turn 订阅、恢复、
   追加输入、取消和事件回放是否支持独立客户端；不得把直接修改 rollout 文件当接口。
2. 探查官方扩展是否允许一个 app-server 被多个客户端安全复用，或必须由 Controller
   建立单一上游连接和多客户端对话代理。
3. 检查本地 Codex CLI 可用的插件/客户端扩展点和版本协商能力；不固定 CLI 版本，只按
   接口能力握手和失败关闭。
4. 在阶段 4、5 已完成的前提下复核 operation ID、幂等账本、Codex 权限模式、写入审批
   和断线结果确认能否携带外部客户端身份，并证明 `full-access` 不会被 Bridge 降级。

实施：

1. Controller 提供仅本机可访问的 Unix socket/Windows named pipe 接口，以短期会话
   凭证和显式用户授权完成接入；不暴露 OpenAI Token、Remote SSH transport token 或
   插件内部长期凭据。
2. 提供会话发现、对话列表、事件回放、thread 续接和能力查询，所有对象保留 thread、
   turn、item、远程主根、序列号和 `originClient`。
3. 支持外部 CLI 提交 turn、取消和介入性回复；VS Code 插件实时投影外部写入、来源、
   状态和错误，且用户可撤销 CLI 接入。
4. 为同一 thread 实施单写者租约或等价串行化机制；同时提交必须明确排队或冲突拒绝，
   重连按 operation ID 去重，不静默覆盖另一客户端的 turn。
5. CLI 发起的远程文件修改只能调用阶段 5 的 Bridge 写入协议，继续执行目标端、
   `expectedHash`、原子替换、幂等和审计；审批直接继承 thread 的 Codex 权限模式，
   `full-access` 自动放行。禁止另开 SSH 或直接写远端。
6. 审计对话读取、turn 写入、项目写入、审批、取消、租约切换和断开，记录外部客户端
...
- VS Code 与 CLI 并发提交不会丢失、重排或重复执行；冲突、断线和结果未知失败关闭。
