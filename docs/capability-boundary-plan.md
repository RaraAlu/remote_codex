...
CodeGraph。新增自动化还证明本地授权根与远端执行器的写入冲突、原子替换、补丁、
重命名、删除、大小限制、符号链接拒绝和审批映射；它们仍不证明真实 Remote SSH
写入、命令取消、断线恢复、双端读取真实窗口链路或完整生命周期。
...
设置恢复、恢复驱动停止、停用空闲态和重新启用也已实测；剩余生命周期和 Windows x64
仍使该版本保持候选状态。
...
1. [x] 增加整文件写入、结构化补丁、创建目录、重命名和删除工具；无
   `expectedHash` 的整文件写入只允许创建。
2. [x] 修改已有文件必须携带 `expectedHash`；新建以目标不存在为前置条件。
3. [x] 通过同目录临时文件、权限继承、刷新和原子替换完成整文件写入。
4. [x] 补丁先在内存中应用并校验基础哈希，再执行原子替换。
5. [x] 覆盖、补丁、重命名和删除接入官方审批，审批绑定目标端、根 ID、路径和摘要；
   有界创建自动执行，`full-access` 自动放行。
6. [x] 两端使用相同错误码处理冲突、权限、部分失败和结果未知。
7. [x] 审计记录目标端、角色、路径、操作、审批、旧/新哈希、字节数和幂等结果，
   不记录正文。

当前进度：`0.3.19` / Executor `0.2.12` 已完成以上实现与自动化。真实 Remote SSH
双端交替写入、断线、权限失败、Windows x64 和 OpenSSH 回退按统一人工清单待补测。
...
- [x] 当前实现作为一个不可拆分的安全目标提交：原子写入、补丁冲突、路径变更、
  审批、审计和幂等必须同时存在，避免中间版本开放不完整写链路。
- 真实 Remote SSH 与双平台证据一个验收提交。
...
全部人工/实机项目已在 2026-07-24 汇总到
`docs/manual-acceptance-backlog.md`。后续源码目标不再逐项等待用户操作；所有条目保持
`待补测`，待实现阶段全部完成并生成最终候选后统一执行。
