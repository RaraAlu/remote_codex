...
| 官方 Codex 扩展 | `openai.chatgpt@26.715.61943` | 已用候选内置运行时启动 Shim；恢复会话、Remote SSH 新任务和普通本地共享附着通过 |
...
| Bridge Controller | `0.3.1` 候选 | 在 `0.3.0` MCP 控制面上增加本地与 Remote SSH 双向实时同 thread 模式；本地双向输入通过，Remote SSH 回归待补测 |
...
| RUNTIME-PASSTHROUGH | 本地窗口使用插件内置 Codex且请求语义不变 | 已实施并实测 | Shim 使用受限运行时指针并在缺失时失败关闭；共享网关不改写本地 `cwd`、权限和审批参数 | Remote SSH 回归仍待验证 |
...
| CLI-INTERVENE | 当前 Codex CLI 对话控制 VS Code Codex 对话 | 自动化候选完成，实机待补测 | 共享网关、持久 MCP、list/read/intervene/interrupt 和跨客户端权限跟踪已实现 | 已运行的旧 CLI 不会热加载 MCP；真实 CLI 工具链路待补测 |
| CLI-BIDIRECTIONAL | CLI 与 VS Code 双向实时共享同一 thread | 普通本地显式与自动入口通过；远端待补测 | 长期自维护入口、普通 `codex` 安全接管、全双工网关、双向流式事件/取消/一端断开测试已完成 | 当前旧 CLI 必须重启；缺少 Remote SSH 与 Windows 实机证据 |
...
真实远端条件文件跳过，154 项通过、5 项跳过；真实官方 app-server 冒烟已覆盖普通
本地窗口和 Remote SSH 窗口的受鉴权外部连接及 MCP 四工具列表，双客户端集成已覆盖
list/read/new-turn/steer/interrupt、跨客户端 `full-access` 权限继承和 `remote_exec`
无二次审批。当前 `0.3.1` 候选待安装；真实本地与 Remote SSH 窗口、当前 CLI 重新加载
MCP、VS Code UI 投影、审计和 Windows x64 待补测。
...
状态变化也会自动触发协调，Shim 内容迁移后自动重载。Remote SSH 回归和 Windows x64
仍待补测。
...
- 官方新任务创建、审批、取消和断线操作必须由用户在 Remote SSH 窗口执行；Controller
  自身升级引起的本地 Shim 迁移已自动重载。
