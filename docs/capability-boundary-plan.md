...
| CLI-INTERVENE | 当前 Codex CLI 对话控制 VS Code Codex 对话 | 当前优先；官方多客户端探查完成 | app-server 已验证多客户端恢复、`turn/steer`、`turn/interrupt` 和事件广播；Codex CLI 支持持久 stdio MCP | 尚无共享网关、长期自维护 MCP 注册、对话控制工具和双客户端实机证据；通用文件写入仍依赖阶段 5 |
...
5. Controller 提供显式启用/撤销命令；首次启用后持久记录托管状态，并在插件升级、
   Shim 内容地址变化或窗口重载时自动刷新 `codex mcp` 注册，形成长期自维护能力。
