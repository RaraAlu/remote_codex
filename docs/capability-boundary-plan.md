...
| Bridge Controller | `0.3.8` 候选 | 无版本值门禁；Linux Remote SSH 官方任务、固定探针、网关正常关闭和设置恢复空闲态通过，Windows 实机待补测 |
...
设置恢复、恢复驱动停止和停用空闲态也已实测；重新启用、剩余生命周期和 Windows x64
仍使该版本保持候选状态。
...
| VERIFY-LIFECYCLE | 设置恢复和进程清理 | 部分通过 | Remote SSH 重载时外部连接正常关闭；设置恢复、恢复驱动停止、旧 Shim/状态清理和重载空闲态实机通过 | 重新启用、独立停止、Extension Host 退出及 relay/MCP 子进程归零待补测 |
