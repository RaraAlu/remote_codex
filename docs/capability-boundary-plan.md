...
| 官方扩展内置 Codex | `0.145.0-alpha.27` | 不满足 Bridge 的精确版本要求，不会被选作后端 |
| 本机 Codex CLI/app-server | `0.145.0` | 已就地升级；活动窗口重载前仍可能保留旧进程 |
| Bridge Controller | `0.2.7` | 当前已安装版本 |
| Remote Executor | `0.2.5` / 协议 v3 | 当前远端工作区执行协议 |
| Remote SSH | `0.124.0` | 当前存在活动的已认证 Remote SSH transport |

仓库已固定 `codexAppServerVersion=0.145.0`，并新增
`protocol/0.145.0/`。Controller 会遍历本机与官方扩展内置 Codex 候选，只有版本与
生成协议完全一致时才进入 `ready`；找不到精确版本时进入 `incompatible`，不会猜测
兼容性或自动降级。
...
- TypeScript 类型检查通过。
- 29 个测试文件通过，1 个真实远端条件测试文件跳过。
- 108 项测试通过，5 项真实远端条件测试跳过，0 项失败。
- Controller、Shim 和 Remote Executor 构建通过。
- `0.145.0` 本地透传、远程窗口启动和线程创建 Shim 冒烟通过。
...
协议生成、线程工作区语义审查、动态工具文本结果兼容测试、真实 app-server 冒烟和
Linux 候选打包已经通过。未知服务端请求白名单仍为 11 项。官方扩展新任务、MCP、
固定远端探针和真实 Remote SSH 链路尚未重跑，因此当前只能视为候选升级。

## 2. 重新汇总的任务清单

| ID | 任务 | 当前状态 | 已有能力 | 主要缺口 |
| --- | --- | --- | --- | --- |
| COMP-145 | 适配 Codex `0.145.0` | 自动化完成，实机待验证 | 本机 CLI、协议、版本门禁、测试、Shim 冒烟和 Linux 候选包已升级 | 当前官方扩展新任务、MCP 和真实 Remote SSH 链路未验证 |
...
- `scripts/generate-protocol.mjs` 可从指定 Codex 生成受控协议子集。
- `test/protocol-compatibility.test.ts` 会逐项比对已知服务端请求，遇到新请求失败关闭。
- Controller 会验证 CLI/app-server 精确版本并自动寻找匹配的本机或官方内置可执行文件。

边界：

- Shim 本身不会二次验证实际子进程版本，安全门禁主要位于 Controller。
- 当前包精确钉住 `0.145.0`；版本不匹配仍会被 Controller 拒绝。
...
1. 本地 Codex/app-server 和远端无 Codex是固定架构，不改为远端安装 Codex。
...
### 阶段 1：Codex `0.145.0` 兼容基线

目标：只升级协议兼容集合，不同时引入双端读写。

进度：本机 CLI、协议生成、版本门禁、定向测试、`npm run check`、Linux 候选打包与
安装已完成，基线提交为 `a3162cf`；下列真实窗口步骤 7-8 和 Windows 产物仍待完成。

实施：

1. 从发布模板创建新的候选记录，填写当前 VS Code、官方扩展、Bridge、Executor、
   Codex、Remote SSH 和 OpenSSH 版本。
2. 使用隔离的 `0.145.0` 可执行文件运行 `npm run protocol:generate`，审查并提交
   `protocol/0.145.0/` 与 `codexAppServerVersion`。
3. 更新协议测试，保持 11 个服务端请求的失败关闭白名单。
4. 增加 `inputAudio` 动态工具结果与恢复历史的兼容测试；Bridge 仍只返回文本时不得
   错误投影或丢弃其他官方内容项。
5. 覆盖 `ThreadStartParams`、`ThreadResumeParams`、turn 环境和
   `runtimeWorkspaceRoots` 的重写测试。
6. 使用 `0.145.0` 运行类型检查、测试、构建、Shim 冒烟和当前平台打包。
7. 安装候选 VSIX 后，由用户在真实 Remote SSH 窗口完成连接、必要重载和新任务创建。
8. 复核 Codex 日志与 Bridge 审计，确认任务到达 Shim、项目操作走远端且本地窗口透传。

验收：

- G0-G3 通过；`0.144.5` 与 `0.145.0` 不匹配时仍失败关闭。
- 当前官方扩展新建、恢复和 turn 均无 `Unknown local project`。
- 未完成真实窗口任务创建前，兼容矩阵不得写成支持 `0.145.0`。
...
1. 在 `0.145.0` 候选上做三组真实实验：远程根写入 `runtimeWorkspaceRoots`、远程根只
   作为 Bridge 逻辑主根、继续使用控制目录。分别验证新建、恢复、turn、附件和当前文件。
...
6. 探查 `0.145.0` permission profile 对本地根的拒绝能力；用 `PreToolUse` hook 拒绝
   Bash、`apply_patch` 和已知本地文件工具，作为第二层防线。
