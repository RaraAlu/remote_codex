...
| CLI-INTERVENE | 本地外部 Codex CLI 接入反代对话并介入写入 | 当前优先；官方多客户端探查完成 | app-server 已验证多客户端恢复、`turn/steer`、`turn/interrupt` 和事件广播；Controller 已有远程 transport、线程改写、权限跟踪和审计 | 尚无共享网关、短期凭证、CLI 续接入口和双客户端实机证据；通用文件写入仍依赖阶段 5 |
...
目标：先让本地运行的外部 Codex CLI 通过 Bridge 受控网关接入官方扩展正在使用的
app-server，并按 thread 的 Codex 权限模式恢复对话、提交 turn、追加输入和取消。
外部客户端继续通过现有 Bridge 动态工具操作远端。通用文件写入仍等待阶段 5，远端
不安装 Codex，系统 CLI 不参与官方 app-server 的发现、选择、透传或回退。
...
3. 官方 CLI 已提供 `codex --remote ws://...` 及 resume/fork 等远程客户端入口；外部
   CLI 仍使用自身正常登录状态，Bridge 不读取或复制其认证文件。
...
4. 提供 Controller 命令和本地启动器，读取活跃网关元数据并以
   `codex --remote` / `codex resume --remote` 接入；不固定外部 CLI 版本，缺少所需远程
   客户端能力时失败关闭。
5. 保存当前 thread 标识和最小连接元数据，使 CLI 能明确续接 VS Code 当前对话；
   对话内容、OpenAI 登录材料和 Remote SSH transport token 均不写入元数据。
6. 依赖官方 app-server 的 `expectedTurnId` 与 turn 状态处理同时提交；Bridge 不另设
