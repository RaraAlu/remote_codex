...
| --- | --- | --- | --- | --- |
| RUNTIME-OFFICIAL | 仅依赖官方扩展内置 Codex | 待实施 | Controller 已能定位官方扩展内置二进制 | 当前候选顺序优先系统 CLI，公开配置仍允许任意路径 |
| RUNTIME-PASSTHROUGH | 本地窗口透传使用插件内置 Codex | 待实施 | Shim 可按环境变量透传指定可执行文件 | 环境缺失时仍回退 PATH/`~/.local/bin/codex` |
| COMP-BUNDLED | 为插件内置 app-server 生成协议 | 待实施 | 协议生成、精确版本和未知请求失败关闭已具备 | 当前协议来自外部稳定版 CLI，不对应插件内置 alpha |
