...
| 本机 Codex CLI/app-server | `0.144.5` | 当前实际后端 |
| Bridge Controller | `0.2.7` | 当前已安装版本 |
| Remote Executor | `0.2.5` / 协议 v3 | 当前远端工作区执行协议 |
| Remote SSH | `0.124.0` | 当前存在活动的已认证 Remote SSH transport |

仓库仍固定 `codexAppServerVersion=0.144.5`，协议文件仍位于
`protocol/0.144.5/`。Controller 会遍历本机与官方扩展内置 Codex 候选，只有版本与
...
- 107 项测试通过，5 项真实远端条件测试跳过，0 项失败。
- Controller、Shim 和 Remote Executor 构建通过。
- `0.144.5` 本地透传、远程窗口启动和线程创建 Shim 冒烟通过。
...
- `openai.chatgpt@26.715.61943` 启动当前 Shim 和本地 `0.144.5` app-server。
- 已有会话能够恢复并开始新 turn。
...
本次只在临时目录安装 `@openai/codex@0.145.0` 并生成 Schema，没有修改全局 Codex、
仓库协议或兼容矩阵。相对 `0.144.5`：
...
因此 `0.145.0` 不是只改版本号即可发布。虽然未知服务端请求白名单无需新增方法，仍需
重新生成协议、审查线程工作区语义、验证动态工具结果投影，并重跑官方扩展、MCP 与
真实 Remote SSH 链路。
...
| --- | --- | --- | --- | --- |
| COMP-145 | 适配 Codex `0.145.0` | 待实施 | 精确版本门禁、协议生成脚本、兼容测试 | Schema 更新、当前官方扩展和真实任务链路未验证 |
...
- 当前包只携带 `0.144.5` 协议；`0.145.0` 仍会被 Controller 拒绝。
