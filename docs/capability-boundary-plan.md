...
访问 Remote Bridge 反代后的对话，并受控介入对话与远程项目写入。远端不安装 Codex，
系统 CLI 不参与官方 app-server 的发现、选择、透传或回退。
...
3. 检查本地 Codex CLI 可用的插件/客户端扩展点和版本协商能力；不固定 CLI 版本，只按
   接口能力握手和失败关闭。
4. 在阶段 4、5 已完成的前提下复核 operation ID、幂等账本、写入审批和断线结果确认
   能否携带外部客户端身份。
...
5. CLI 发起的远程文件修改只能调用阶段 5 的 Bridge 写入协议，继续执行目标端、根 ID、
   路径约束、`expectedHash`、原子替换、审批、幂等和审计；禁止另开 SSH 或直接写远端。
