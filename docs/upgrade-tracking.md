...
- 所有版本值只用于诊断、证据和触发回归链。运行时不得因版本值不同、未知或缺失而
  拒绝；只有实际可执行文件、所需能力、消息结构或真实操作失败才能阻断。
...
| Remote Executor 或协议变化 | 内嵌 VSIX 一致性、能力握手、工作区根校验、远端读/搜/Git/命令、stdio 生命周期 | Executor 版本、能力集合、诊断、远端操作审计 |
...
| 编号 | 链路 | 通过条件 |
| --- | --- | --- |
| G0 | 诊断与能力 | 记录全部组件版本但不以任何版本值门禁；Codex 通过真实 app-server 操作验证，Executor ping 声明完整所需能力；仅能力、消息结构或真实操作失败时进入 `incompatible` |
...
尚未实现的产品能力，例如哈希保护写入和断线恢复，使用 `不适用` 并链接
`docs/implementation-status.md`。运行中取消已进入自动化候选，因此该项必须记录
自动化与真实 Remote SSH 结果；真实取消未执行时只能写 `待补测`，不能写 `通过`。
安全硬门禁 G7 不能因功能未实现而省略。
...
| 运行中取消 | `turn/interrupt` 到远端进程树消失并收到 `CANCELLED` 的时间 | 至少 3 次 | 3/3 明确取消；遗留进程为 0；不得返回成功或自动重放 |
| 幂等重放 | 同一幂等键重复提交后的副作用次数、结果一致性和冲突拒绝 | 至少 5 次 | 副作用恰好 1 次；终态一致；参数变化 100% 拒绝 |
| 断线结果确认 | transport 中断到 completed/cancelled/failed/unknown 明确结果的时间和副作用次数 | 每种状态至少 5 次 | 原命令不重放；终态与账本一致；unknown 或查询不可达明确返回 `RESULT_UNKNOWN` |
...
取消确认耗时从 `0.3.15` 起为必填指标；幂等键重复副作用数从 `0.3.16` 起、断线
结果确认从 `0.3.17` 起为必填指标。未安装候选到真实 Remote SSH 窗口时写 `待补测`。
尚未实现的写入哈希继续列出 `不适用（能力未实现）`；后续开始实现时追加写入冲突
拒绝率，不能复用只读链路结果。
