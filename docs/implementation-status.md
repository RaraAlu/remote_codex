...
阶段 C 尚未关闭。0.1.8 提供与官方权限模式一致的远程命令执行；写操作、取消、断线恢复
和本地执行硬阻断完成前，不得用于无人值守的有副作用任务。

## 本地 MCP 边界

远程工作区任务仍可使用本地 Codex app-server 原有的 MCP、App 和 Connector 服务，
Bridge 不会移除或远程安装这些服务。当前已确认本地 MCP 进程与 Remote Bridge
app-server 共存。任意 MCP 工具不能在缺少工具语义时被透明重定向；需要直接读取
远端文件的本地 MCP 服务应自行支持 SSH，项目命令使用 Bridge 的 `remote_exec`。
