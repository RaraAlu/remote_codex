...
| 独立本地审计日志和脱敏 | 已实现并测试 |
| Bridge 工具原生界面投影 | 已实现并测试；真实窗口观感待 0.1.7 实机确认 |
...
| 官方命令审批 | 已实现；每次显示主机、规范化 `cwd`、完整命令和环境变更 |
| 命令输出流 | 已映射为 `item/commandExecution/outputDelta` |
| 审批绑定 | 当前审批仅匹配一个待处理调用 ID，不接受“本会话始终允许” |
| 运行中取消 | 未实现 |
| 哈希保护写入和补丁 | 未实现 |
| 断线结果确认和幂等 | 未实现 |
| Core 内置本地工具硬阻断 | 未实现 |

阶段 C 尚未关闭。0.1.7 先提供受审批命令以完成实机交互验证；写操作、取消、断线恢复
和本地执行硬阻断完成前，不得用于无人值守的有副作用任务。

## 本地 MCP 边界

本地 Codex app-server 仍可使用原有本地 MCP 服务，Bridge 不会移除或远程安装这些
服务。任意 MCP 工具不能在缺少工具语义时被透明重定向；需要远端资源的本地 MCP
服务应自行通过 SSH 访问，项目命令使用 Bridge 的 `remote_exec`。
