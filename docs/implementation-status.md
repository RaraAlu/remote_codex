...
状态：能力探查完成，提升为当前优先实施。本地外部 Codex CLI 将通过 Bridge 受控网关
接入官方扩展内置 Codex 所运行的同一 app-server，恢复 VS Code thread、提交 turn、
追加输入和取消；外部客户端的远程工具调用仍经过既有请求改写、权限跟踪和活动
VS Code Remote SSH transport。通用项目文件写入等阶段 5 后再接入，不增加临时旁路。
...
2026-07-23 实测确认，官方 app-server 的两个独立 WebSocket 客户端可同时初始化并接收
事件；第二客户端能恢复第一客户端的 thread，对同一活动 turn 成功调用
`turn/steer` 和 `turn/interrupt`。官方 CLI 已有 `--remote` 客户端入口，因此不再自造
对话协议或强制单写者租约。当前实现缺口收敛为：共享上游生命周期、loopback 短期鉴权、
每客户端 Shim 代理、活跃 thread 发现、CLI 启动入口以及双客户端真实窗口证据。
