...
## TODO：Codex 原生上下文入口扩展

状态：待实现。首份能力探针基于本机实际加载的官方扩展 `26.721.41059`，不把该版本
设为运行时门禁。
...
1. 在 VS Code 资源管理器右键菜单增加“添加到 Codex 上下文”，支持单文件、多文件、
   单文件夹和多文件夹，并同时覆盖普通本地窗口与 Remote SSH 工作区。
2. 文件夹按稳定顺序展开和去重；遵守工作区排除规则、忽略文件、符号链接边界、二进制
   与大小上限。超量输入必须在读取或发送前提示，不得静默截断成不完整上下文。
3. 支持把一个或多个本机文件夹拖入 Codex 输入区；保留官方现有的单文件和多文件拖放。
4. Remote SSH 窗口支持把本机文件拖入当前 Codex 上下文。上下文必须保留
   `local` / `remote` 来源身份，不能把本机路径解释为远端路径，也不能伪造或替换
   VS Code 工作区 URI。
5. 优先使用官方命令、VS Code 命令参数和 app-server 协议完成资源管理器入口。若文件夹
   拖放最终必须修改官方 Webview，则实现独立、可关闭、可恢复的兼容适配层：按实际
   资产结构探测能力，幂等备份和恢复，升级后失败关闭并保留 Bridge 核心功能，诊断中
   明确报告补丁状态；不得按官方扩展版本号直接接纳运行时。
6. 自动化覆盖选择集合、文件夹展开、去重、排除、大小限制、路径来源和升级后失败关闭；
   实机矩阵至少覆盖本地单/多文件、本地单/多文件夹、本机文件夹拖放、Remote SSH
   远端资源右键，以及 Remote SSH 窗口中的本机文件拖放。

退出条件：所有入口把内容加入当前活动 thread，重复资源只出现一次；本机和远端同名
文件保持可区分；窗口重载、官方扩展升级和功能撤销不会破坏官方 Codex UI 或 Bridge
主链路。实机开始后把逐项结果转入 `docs/manual-acceptance-backlog.md`，候选证据写入
新的日期化 `docs/acceptance/` 文件。
