...
- Bridge 不注册资源管理器上下文添加命令，也不为拖放创建来源清单、本机副本或远端
  ...
  URI/文件路径并调用官方文件上下文命令。VS Code Explorer 载荷必须带有 Workbench
  专用 MIME 证据，Bridge 才为文件上下文命令附加受管路径标记；按实际消息处理器和
  Composer 能力形状修改的唯一官方 Webview 资产只把该标记转换为当前光标处的原生 `@`
  引用，并在写入节点前移除标记。没有专用证据的系统文件管理器载荷保守走未标记的官方
  附件路径，普通 `chatgpt.addFileToThread` 的语义也不改变。首次启用必须由
  用户执行命令并确认；扩展激活不会静默提权。Linux 系统安装使用 polkit，把
  用户态暂存文件复制为目标目录内的 root 临时文件后同时校验替换内容和当前目标
  ...
  活动工作区资源；Remote SSH 资源必须属于当前规范根。系统文件管理器可以由用户显式
  拖入工作区外的本机路径，但 Bridge 只执行存在性/类型检查并交给官方附件通道，不读取
  正文、不转成远端路径，也不把它伪装成工作区内 `@` 引用。
