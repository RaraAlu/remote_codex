...
4. 用户确认本次允许访问的远程根目录。
5. Bridge 扩展配置或引导配置本地 Codex 后端入口。
...
| --- | --- | --- | --- |
| DEC-001 | 是否允许在本地 Ubuntu 安装配套 CLI Shim | 允许，它是复用官方界面的必要入口 | 阻塞 |
...
| DEC-006 | 是否允许 Bridge 临时修改并在退出时恢复 `remote.extensionKind` | 允许，但必须预览、备份和恢复 | 阻塞 |
| DEC-007 | 审计日志默认保留周期 | 建议 7 天，可手动清理 | 不阻塞 |

在 DEC-001、DEC-005 和 DEC-006 未确认前，可以进行只读技术探针，但不应进入正式实现。
