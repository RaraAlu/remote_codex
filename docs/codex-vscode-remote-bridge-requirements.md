...
| --- | --- |
| 文档状态 | 可行性验证详细需求基线 |
| 版本 | 0.2 + Windows/官方扩展运行时附录 |
| 日期 | 2026-07-22 |
| 目标仓库 | `/home/zkbot/work/train/MimicLite` |
| 本地主机 | 可联网 Windows x64 或 Ubuntu x64，运行 VS Code、官方 Codex 扩展及其网络请求 |
| 远程主机 | 离线 Ubuntu，保存代码并执行开发任务 |

本文档定义一个 VS Code 桥接扩展的需求。该扩展应让用户在本地主机上保留 VS Code 和官方 Codex 的使用习惯，同时让文件、命令、Git、测试和 GPU 任务实际运行在另一台无法访问公网的远程 Ubuntu 主机上。

### 1.1 Windows 实现附录

0.2.0 将原始基线中的“本地 Ubuntu”泛化为“本地主机”，支持 Windows x64 和
Ubuntu x64；远程执行目标仍为 Ubuntu。两个本地平台使用同一个扩展 ID 和代码库，
但分别发布 `win32-x64` 与 `linux-x64` VSIX。Windows 使用原生 Node SEA Shim，
Linux 使用 CJS Shim；Codex 运行时只从 VS Code 当前加载的官方扩展目录获取，不固定
官方扩展版本，也不发现或回退系统 Codex CLI；OpenSSH、配置目录和其他可执行文件路径
必须按本地平台发现，不得把开发机或另一平台的绝对路径写入扩展设置。默认执行通道改为运行在 Remote
Extension Host 中的轻量 Executor，以复用 VS Code 已认证连接；独立 OpenSSH 为回退。
Executor 不包含 Codex、模型逻辑或用户凭据。下文出现的“本地主机”均遵循此定义。
...
- 本地主机可以访问公网，并已安装 VS Code 和官方 Codex 扩展。
...
1. Codex 的登录、会话、模型请求和额度消耗发生在本地主机。
...
- ChatGPT 登录和 OpenAI 网络流量仅出现在本地主机。
...
| INV-002 | ChatGPT/Codex 凭据永不离开本地主机。 |
| INV-003 | OpenAI 请求永不从远程 Ubuntu 发出。 |
| INV-004 | 项目 Shell、Git、测试和 GPU 命令永不在本地主机执行。 |
...
| 组件 | 运行位置 | 职责 | 是否允许联网 |
| --- | --- | --- | --- |
| VS Code 桌面端 | 本地主机 | 编辑器窗口、Remote SSH UI、Diff 和命令面板 | 是 |
| 官方 Codex 扩展 | 本地扩展宿主 | 提供官方 Codex 对话与审批界面 | 是 |
| Bridge 扩展 | 本地扩展宿主 | 配置、状态、路径映射、策略和生命周期管理 | 是 |
| CLI Shim | 本地主机 | 兼容 Codex CLI 启动参数，代理 app-server JSON-RPC | 是 |
| Codex app-server | 本地主机 | ChatGPT 登录、会话、模型调用和工具编排 | 是 |
| Remote Executor | 本地主机 | 将结构化文件和命令请求转换为 SSH/SFTP 操作 | 仅访问远程 SSH |
...
- 官方 Codex 扩展和 Codex 后端运行在本地主机。
- 远程 Ubuntu 仅需已有的 OpenSSH、Shell 和项目自身工具链。
- 所有 ChatGPT 凭据、访问令牌和会话状态保存在本地主机。
...
| 远程 Ubuntu | 本地主机 | 新建反向连接 | MVP 禁止 |

MVP 不依赖 SSH 反向隧道。所有连接均由本地主机主动发起，远程端不需要发现或访问本地端地址。
...
1. 用户在本地主机安装官方 Codex 扩展和 Bridge 扩展。
...
| 编号 | 优先级 | 需求 |
| --- | --- | --- |
| AUTH-001 | P0 | 使用本地主机的 ChatGPT 登录状态和套餐内 Codex 额度。 |
| AUTH-002 | P0 | 不要求用户提供 OpenAI API Key。 |
| AUTH-003 | P0 | OpenAI 请求必须从本地主机发出。 |
...
| EXEC-006 | P0 | Bridge 不可用时不得在本地主机上执行原命令。 |
...
| OBS-006 | P1 | 每个候选版本记录测试计数、关键链路延迟、固定探针成功率、额外认证、错误执行位置、遗留进程和产物身份。 |
...
| 编号 | 优先级 | 需求 |
| --- | --- | --- |
| LIFE-001 | P0 | 启动时检测官方扩展身份、CLI Shim 和内置 app-server 协议。 |
...
| LIFE-008 | P1 | 兼容集合中任一组件变化时保存独立的发布跟进记录，包含触发链路、硬门禁、量化指标、证据和遗留风险。 |
| LIFE-009 | P1 | Windows x64 与 Linux x64 的打包、Shim、官方任务和 Remote SSH 运行结果必须分别记录，不得用跨平台打包代替运行时验收。 |
...
- Bridge 应固定官方扩展身份而非版本号，记录实际扩展版本，并检查其内置
  app-server 与已验证协议兼容；系统 Codex CLI 不属于兼容集合。
...
审批设为允许。服务端未注册的工具、显式 allowlist、单工具规则和托管策略仍是上层
边界，不得伪造工具或绕过托管策略。

该路由和访问策略只覆盖当前 app-server 进程的 MCP 配置，不得修改全局
...
- Codex app-server、ChatGPT 登录和模型网络位于本地主机。
- Remote Executor 位于本地主机，通过 SSH 主动连接远程 Ubuntu。
...
- 从当前安装的官方扩展内置 Codex 运行 app-server 协议生成命令。
...
1. 在本地主机登录 ChatGPT，远程 Ubuntu 不配置任何 OpenAI 凭据。
2. 在远程 Ubuntu 执行 `command -v codex`，结果应为空。
3. 发起 Codex 任务，确认 OpenAI 网络连接来自本地主机。
...
5. 模拟官方扩展缺失、身份错误或内置 app-server 协议不匹配，确认 Bridge 进入
   `incompatible`；仅扩展版本号变化不得触发拒绝。
...
| 本地诱饵文件哈希 | 证明没有本地错误写入 |
| SSH 断线测试记录 | 证明系统安全失败 |
| 发布升级跟进记录 | 证明组件触发项、分平台门禁、量化回归和声明范围已经闭环 |
...
- 单个本地主机（Windows x64 或 Ubuntu x64）。
- 单个远程 Ubuntu SSH 主机。
- 单个远程工作区根目录。
- SSH Key/Agent 认证。
- 远程文件读取、搜索、补丁和写入。
...
- 按 `docs/upgrade-tracking.md` 保存分平台硬门禁、量化指标和不可覆盖的候选版本证据。
...
- CLI Shim 的 Windows x64 原生可执行文件和 Linux x64 CJS 入口。
...
| --- | --- | --- | --- |
| DEC-001 | 是否允许在本地主机安装配套 CLI Shim | 已确认允许，它是复用官方界面的必要入口 | 已确认 |
