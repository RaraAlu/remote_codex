...
| 本地主机 | 可联网 Windows x64 或 Ubuntu x64，运行 VS Code、Codex 和网络请求 |
...
Linux 使用 CJS Shim；Codex、OpenSSH、配置目录和可执行文件路径必须按本地平台发现，
不得把开发机或另一平台的绝对路径写入扩展设置。默认执行通道改为运行在 Remote
