...
- M01：当前 `0.3.31` 候选热启动 3/3 通过，P50 为 3,619 ms、最大值为
  3,927 ms。`0.3.30` 曾完成冷启动 2/3、官方 UI 直接新建和恢复各 3/3；因 Controller
  已更新，这些继续保留为历史证据，不替代 `0.3.31` 的最终当前候选样本。当前候选冷
  启动、官方 UI 任务创建与恢复将分别在 L03-L07 中重新采集。
...
- Linux Controller `0.3.31` 与 Executor `0.2.16` 已安装并在 `g1_1` 重载；
  M01 当前候选热启动 3/3 已完成，L02 已完成。`0.3.30` 的冷启动和官方直接任务保留
  为历史证据；`0.3.31` 的当前候选样本将在剩余批次中重新采集。

## 当前 Linux 剩余人工批次

以下只列当前 `0.3.31` Linux 候选无法由注入、协议探针或静态检查替代的人工动作。
每次用户完成动作后，Codex 负责读取日志、审计、会话和远端进程，执行其余工具矩阵并
更新证据。Windows x64 和未选择的 OpenSSH 回退不在本轮。

### L01 当前候选启动与官方任务样本

- 已完成：`0.3.31` 当前候选热启动 3/3，P50 为 3,619 ms、最大值为 3,927 ms、
...
- 待补测：在 L03/L04 的官方 UI 矩阵中收集 `0.3.31` 直接新建和恢复样本；在 L07
