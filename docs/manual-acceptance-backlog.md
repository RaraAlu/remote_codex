...
- M01：`0.3.31` 候选热启动 3/3 通过，P50 为 3,619 ms、最大值为 3,927 ms；
  ...
  `0.3.32`，最终精确产物热启动 1 次为 3,231 ms；同版本功能候选的官方 UI 直接新建
  1 次通过，但最终哈希仍未达到当前候选最低样本。其余冷/热启动、直接新建和恢复仍在
  L03-L07 采集。
- M02：`0.3.30` 实机注入的 `fs/readFile`、`command/exec`、`thread/shellCommand`
  ...
  完整文件；最终官方 UI 直接任务、本地次级根虚拟资源隔离均通过。本地 Core 同名
  诱饵和普通本地窗口透传仍待人工确认。
...
- 已完成：`0.3.32` 最终精确产物热启动 1 次为 3,231 ms；最终失败关闭加固前的同版本
  功能候选官方面板直接新建 1 次，在 32 ms 内进入 turn-start，9,020 ms 完成，没有
  `Unknown local project`。最终精确产物另由注入 turn 完成自动选区复核。
...
- 已完成：注入任务分别验证自动远端选区和自动完整文件；同版本功能候选的官方 UI
  直接新建任务在无附件、无 Bridge 命令时准确得到 `g1_1` 相对路径、真实 URI、选区
  范围和正文；最终加固构建已重跑自动选区注入，最终哈希官方 UI 样本仍待补。
