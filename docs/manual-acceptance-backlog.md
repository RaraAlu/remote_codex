...
socket 关闭挂起修复、写入失败完整性和当前候选固定探针。
...
- M01：`0.3.31` 候选热启动 3/3 通过，P50 为 3,619 ms、最大值为 3,927 ms；
  ...
  `0.3.34`，当前精确产物热启动 1 次为 3,812 ms；`0.3.32` 最终精确产物的官方 UI
...
- M03：`g1_1` 的远端读取、目录树、搜索和 `pwd` 各 5 次全部成功并记录 P50/最大值；
  ...
  恢复；L02 已收口。`0.3.34` 当前候选重新采集读取、目录树、有效查询搜索、Git 和
...
- M04：`g1_1` 的远端目录创建、文件写入、精确补丁、重命名和清理闭环通过；过期哈希
  ...
  均不变且临时文件为 0。Executor 独立失联写入仍待补。
...
- Linux Controller `0.3.34` 与 Executor `0.2.16` 已安装并在 `g1_1` 重载；L02、
  自动 IDE 背景、外部 `full-access` 和 transport socket 关闭挂起修复均已闭环。
  `0.3.30` 至 `0.3.33` 的启动和官方任务保留为历史证据；`0.3.34` 的剩余当前候选
  样本将在后续批次中重新采集。

## 当前 Linux 剩余人工批次

以下只列当前 `0.3.34` Linux 候选无法由注入、协议探针或静态检查替代的人工动作。
...
- 已完成：`0.3.34` 最终精确产物热启动 1 次为 3,812 ms，Remote SSH Shim 来自当前
  安装候选。`0.3.32` 最终精确产物官方面板直接新建 1 次，在 84 ms 内创建
