...
- M01：当前 `0.3.30` 候选热启动 3/3 通过，P50 为 3,709 ms、最大值为
  3,748 ms；冷启动完成 2/3，最后一次与 L07 带载窗口关闭合并。官方 UI 直接新建和
  直接恢复各 3/3 通过，任务创建 P50 为 82 ms、最大值为 1,601 ms，且无
  `Unknown local project`。当前候选结果不复用 `0.3.26` 历史样本。
- M02：`0.3.30` 实机注入的 `fs/readFile`、`command/exec`、`thread/shellCommand`
  和未知 `fs/` 风险方法均返回 `-32003` 并进入失败审计；成功本地项目操作为 0。
  官方 UI 附件、本地同名诱饵和普通本地窗口透传仍待人工确认。
- M03：`g1_1` 的远端读取、目录树、搜索和 `pwd` 各 5 次全部成功并记录 P50/最大值；
  在不覆盖既有 `.git` 的隔离前置条件下，远端 `workspace_git_status` 也达到 5/5，
  P50 为 81 ms、最大值为 93 ms，清理后主根恢复为非 Git 状态。本地次级根选择、重载
  持久化、双端交替和授权撤销仍待补。
- M04：`g1_1` 的远端目录创建、文件写入、精确补丁、重命名和清理闭环通过；过期哈希
  ...
  CLI 后，turn 自动中断且文件仍为原哈希、临时文件为 0。本地端、原始写入大载荷、
  Executor 独立失联及窗口 transport 断开矩阵仍待补。
...
- Linux Controller `0.3.30` 与 Executor `0.2.16` 已安装并在 `g1_1` 重载；
  M01 当前候选的热启动、官方直接新建和官方直接恢复最低样本已完成，仅余与 L07 合并
  的最后一次带载冷启动。

## 当前 Linux 剩余人工批次

以下只列当前 `0.3.30` Linux 候选无法由注入、协议探针或静态检查替代的人工动作。
每次用户完成动作后，Codex 负责读取日志、审计、会话和远端进程，执行其余工具矩阵并
更新证据。Windows x64 和未选择的 OpenSSH 回退不在本轮。

### L01 当前候选启动与官方任务样本

- 已完成：当前候选热启动 3/3，P50 为 3,709 ms、最大值为 3,748 ms、失败数为 0。
- 已完成：官方 Codex 面板直接新建任务 3/3、恢复原 thread 3/3；任务创建 P50 为
  82 ms、最大值为 1,601 ms，`Unknown local project` 命中为 0。
- 已完成：完整关闭并重新打开 `g1_1` Remote SSH 窗口 2 次，两次均恢复 `ready` 且
  旧进程和会话文件已清理。
- 待补测：最后一次冷启动与 L07 的带载窗口关闭合并，不额外重复关闭；完成前 L01
  总体仍保持 `待补测`。

### L02 本地次级根授权

- 在命令面板执行 `Codex Bridge: Authorize Local Root`，选择专用本地测试 Git 目录，
  接受一次 Reload；确认诊断中出现 `local/secondary` 且重载后仍存在。
- Codex 随后自动完成同一 task 的本地/远端交替读取、树、搜索、Git、写入、补丁、
  重命名、删除、冲突、权限和大小上限矩阵。
- 在命令面板撤销该根；Codex 验证既有执行器和新请求立即失败关闭。再授权同一目录，
  由 Codex 验证恢复后清理测试目录。

### L03 官方界面上下文与本地 Core 诱饵

- 在官方 UI 分别附加当前远程文件、普通附件和当前编辑器文件，确认远程主根、URI、
  当前文件和选区均来自 `g1_1`，本地控制目录不显示为项目根。
- 授权根和控制目录放置同名无害诱饵后，在 Remote SSH task 中要求读取、搜索、修改和
  执行；人工观察 UI，Codex 核对成功本地项目操作仍为 0。
- 另开普通本地 VS Code 窗口执行一个正常本地 task，确认 Remote SSH 的拒绝策略没有
  污染本地项目语义；完成后关闭该窗口，避免影响 L06 的唯一会话测试。
