...
- Linux Controller 已安装并重载为 `0.3.38` 候选，Executor 保持 `0.2.19`；
  `0.3.37` 的精确安装、重载、能力升级和独立失联写入已经闭环。L02、自动 IDE
  背景、外部 `full-access` 和 transport socket 关闭挂起修复均已闭环。`0.3.30`
  至 `0.3.37` 的启动、注入和官方任务保留为历史证据；`0.3.38` 中断工具历史终态
  实机复核以及剩余官方 UI 样本将在后续人工批次中采集。

## 当前 Linux 剩余人工批次

以下只列当前 `0.3.38` Linux 候选无法由注入、协议探针或静态检查替代的人工动作。
