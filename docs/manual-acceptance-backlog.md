...
- M01：`0.3.31` 候选热启动 3/3 通过，P50 为 3,619 ms、最大值为 3,927 ms；
  ...
  `0.3.32`，最终精确产物热启动 1 次为 3,231 ms；最终精确产物的官方 UI 直接新建
  1 次为 84 ms 创建、5,984 ms 完成，自动选区正确。其余冷/热启动、直接新建和恢复
  仍在 L03-L07 采集。
- M02：`0.3.30` 实机注入的 `fs/readFile`、`command/exec`、`thread/shellCommand`
  ...
  专用工具诱饵和当前候选普通本地窗口项目操作仍待人工确认；本地外部 `full-access`
  已发现独立消息兼容缺口。
...
- Linux Controller `0.3.32` 与 Executor `0.2.16` 已安装并在 `g1_1` 重载；
  L02 已完成，自动 IDE 背景目标已闭环。`0.3.30` / `0.3.31` 的启动和官方任务保留
  为历史证据；`0.3.32` 的剩余当前候选样本将在后续批次中重新采集。

## 当前 Linux 剩余人工批次

以下只列当前 `0.3.32` Linux 候选无法由注入、协议探针或静态检查替代的人工动作。
...
- 已完成：`0.3.32` 最终精确产物热启动 1 次为 3,231 ms；最终精确产物官方面板直接
  新建 1 次，在 84 ms 内创建 conversation、5,984 ms 完成，自动选区正确且没有
  `Unknown local project`。同版本失败关闭加固前的功能候选另有一次 9,020 ms 样本。
...
- 已完成：同名 Bridge 工具诱饵默认读取、搜索、补丁恢复和 `pwd` 全部落到
  ...
  该结果不替代 Core 专用工具负测。
- 诊断：普通本地 `on-request` 外部新建 turn 可完成；同一旧本地会话的
  `full-access` 因无效权限档案失败，且尚未在当前候选 Shim 上执行真实本地项目操作。
