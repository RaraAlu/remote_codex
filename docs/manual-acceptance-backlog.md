...
输出且无超时，原 `119,489 ms` 重入等待已关闭。外部 MCP 使用
`clientInfo.name=codex_vscode_bridge_mcp` 的冷初始化仍会约 30 秒超时，中性名称只需
`5–6 ms`，该身份兼容项继续保留在 README；完整 M01-M11 重跑、双平台 stage 收集和量化
门禁仍是后续 `0.4.0` 决策前的活动前置。
