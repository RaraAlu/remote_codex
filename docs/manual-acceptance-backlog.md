...
- M03：远端读取、Git 状态和 `pwd` 各成功一次，前后 Git 状态一致；目录树、搜索、
  本地次级根和每项 5 次样本仍待补。
- M04：`full-access` 下完成一次远端创建、哈希覆盖、读取和删除闭环；本地端、过期
  哈希冲突、其他变更类型、错误和断线矩阵仍待补。
- M06：附着 CLI 方向完成一次运行中取消并确认目标进程消失；官方 UI 方向、需审批
  模式和每方向 3 次样本仍待补。
- M08：`g1_1` 完成一次后台启动、状态、增量日志、运行中取消和取消后复查；成功、
  失败、超时、截断、客户端断开和窗口关闭矩阵仍待补。
- M09：`g1_1` 启动审计为 `remoteMcpServers=["codegraph"]`，经 VS Code transport
  完成一次 `codegraph_status`，耗时 29 ms、`isError=false`；三次初始化和五次固定调用
  仍待补。
- M10：外部 MCP 方向完成一次新 thread、steer、取消、历史观察和安全写入；官方 UI
  反向操作、冲突、断开、重启、描述符过期和权限撤销仍待补。
