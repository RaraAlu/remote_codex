...
- 完整关闭并重新打开 `g1_1` Remote SSH 窗口 3 次，补当前候选冷启动 3/3；最后一次
  与 L07 的带载窗口关闭合并，不额外重复关闭。
- 在当前窗口再执行 2 次 Window Reload，连同安装 `0.3.30` 后已记录的 3,748 ms
  样本，补热启动 3/3。
- 每次进入 `ready` 后，从官方 Codex 面板直接新建任务 3 次、恢复历史任务 3 次。
  `0.3.26` 的历史样本只作比较，不计入当前候选最低样本。
- 人工只负责打开、重载、新建和恢复；P50、最大值、失败数、Shim 命中和
  `Unknown local project` 检查由 Codex 从日志复核。
