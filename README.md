...
- 协议子集由本机 Codex `0.145.0` 生成，运行时要求精确版本匹配。
...
- [ ] 将官方 `openai.chatgpt` 扩展及其内置 Codex 设为 Bridge 唯一 app-server 来源；
  系统 `codex` CLI 不再参与发现、版本选择或运行时回退。
- [ ] 适配 Codex CLI/app-server `0.145.0`：本机 CLI、协议子集、精确版本门禁、自动化
  和 Linux 候选包已完成，但该方案已被“官方扩展内置运行时为版本权威”的目标取代。
