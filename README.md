...
- 本机 VS Code 与远端中文语言包已对齐到 `1.131.0` / `1.131.2026072717`。远端失效的
  `127.0.0.1:32081` HTTP/HTTPS/ALL proxy 已在保留系统配置备份后停用；首轮修复后重载
  没有语言包重复更新、Remote Extension Host 无响应或额外自动重载，Executor 能力探测
  从 `8,096 ms` 降至 `1,182 ms`。当前计为修复后的第 1/3 次稳定重载；退出条件为再完成
  两次普通窗口重载，三次均无重复更新、无响应或额外自动重载，且能力探测保持低于 2 秒。
