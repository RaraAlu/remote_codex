...
> 当前 `0.3.42` 是 Linux x64 / `g1_1` Remote SSH 候选，不是最终发布版。运行中取消、
...
失败候选，并由 `0.3.42` 完成线程、配置和权限档案列表投影。只有本节活动项全部
完成后才更新根包为 `0.4.0`。本次远端写入完整性修复更新
Remote Executor 为 `0.2.19`，诊断协议为 11。
...
- [ ] 从官方 UI 取消运行中长命令达到 1/3；首个样本在人工允许后由官方 UI 停止，
  远端执行 8,906 ms 后以 `CANCELLED` 终结，取消请求到终态为 97 ms。再补 2 次并在
  每次终态后立即确认远端进程树归零。
