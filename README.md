...
> 当前 `0.3.32` 是 Linux x64 / `g1_1` Remote SSH 候选，不是最终发布版。运行中取消、
...
本节是当前唯一权威 README 代办。执行目标限定为 Linux x64 Controller、Linux Shim、
远端 Ubuntu Executor，以及 `g1_1:/home/unitree/mimiclite-sim2real` 的真实 VS Code
Remote SSH 链路。能由注入、协议探针和静态检查完成的步骤由 Codex 执行；只有官方 UI、
窗口关闭、视觉确认、权限切换和独立失联操作需要用户参与。

版本按实现范围推进：`0.3.32` 保留自动 IDE 背景证据；普通本地会话
`full-access` 若产生行为修复，先形成独立 `0.3.33` 目标；只有本节活动项全部完成后
才更新根包为 `0.4.0`。Remote Executor 未发生实现或协议变化时保持 `0.2.16`。

### 实现与证据

- [ ] 将最终精确 `0.3.32` 官方自动 IDE 背景样本、L03 同名诱饵默认远端路由、
  双向补丁恢复、本地 `on-request` 诊断和审计零泄漏写入验收记录并提交。
- [ ] 修复普通本地会话从外部入口选择 `full-access` 时发送无效权限档案的问题；
  按能力和消息形状兼容，不使用组件版本作为准入条件，并补本地/Remote SSH 回归。
...
- [x] 最终精确 `0.3.32` Linux VSIX 已安装；自动选区、完整文件、本地资源隔离通过。
- [x] 最终精确产物官方直接新建 1/3：创建 84 ms，turn 5,984 ms，自动选区正确。
- [ ] 当前最终候选热启动达到 3/3；`0.3.32` 当前为 1/3。
- [ ] 当前最终候选冷启动达到 3/3；`0.3.32` 当前为 0/3。
- [ ] 当前最终候选官方直接新建达到 3/3、恢复达到 3/3；`0.3.32` 当前分别为
  1/3 和 0/3。检查当前 Shim、规范远端根和 `Unknown local project=0`。

### L02-L03 根、上下文与 Core 诱饵

- [x] 本地次级根选择、持久化、双端读写、撤销失败关闭和重新授权闭环通过。
- [x] 自动远端选区、自动完整文件、真实 `vscode-remote` URI 和正文不入审计通过。
- [ ] 在官方 Remote SSH task 中实际尝试读取、搜索、修改和执行本地控制目录及授权根
  的同名诱饵；成功本地项目操作必须为 0，远端文件必须恢复初始哈希。
- [ ] 在使用当前候选 Shim 的普通本地 VS Code 窗口执行真实本地读取、Git 和安全命令，
  确认 Remote SSH 拒绝策略没有污染本地项目语义。
- [ ] 当前候选固定远端读取、目录树、搜索、Git 和 `pwd` 各完成至少 5 次，成功率
  100%，审计目标端、根 ID、角色、规范路径和 `remoteCwd` 全部一致。
...
- [ ] Codex 准备并打开本地/远端同名文件、选区和 Diff；用户确认焦点、行列定位、
  文件名、目标端、路径及 Diff 左右端身份。
- [ ] Codex 触发过期快照、越界路径、已撤销根和过期资源；用户确认错误提示不会误开
  本地文件，随后由 Codex 复核审计并清理诱饵。

### L06 托管入口与权限撤销

- [ ] 用户关闭其他 Bridge 活动窗口，只保留 `g1_1`；Codex 验证普通 `codex` 无参数
  自动附着唯一会话。
- [ ] 用户停用外部 CLI/MCP 集成；Codex 验证旧描述符、端点和权限立即失效。重新启用
  后只生成当前会话入口。

### L07 Executor 失联与最终窗口关闭

- [ ] 用户触发一次 Remote Executor 独立失联；Codex 在失联前后覆盖运行中命令、写入、
  后台任务和 stdio MCP，核对失败关闭、`RESULT_UNKNOWN`、原文件完整和无重放。
- [ ] Codex 准备前台、后台、写入和 MCP 带载样本后，用户关闭整个 Remote SSH 窗口并
  重开；Shim、relay、MCP、任务、远端进程、会话文件、socket 和旧账本必须清理。
- [ ] 重开后执行最终官方 MimicLite task、固定 MCP 调用和安全扫描；日志、审计、
  进程参数、MCP 配置、远端环境和仓库中的敏感信息命中必须为 0。

### `0.4.0` 发布门禁

- [ ] 对最终源码运行定向测试、`npm run check`、Linux 构包、包内容和 SHA-256 核对；
  `dist/` 只保留当前 Linux Controller、当前版本化 Executor、无版本 Executor 和构包
  所需当前输出。
- [ ] 按 G0-G9 记录测试计数、启动/任务/MCP P50 与最大值、失败数、额外认证数、错误
  执行位置、遗留进程数和产物身份；没有测量的数据不得写成 0。
- [ ] 更新根包和锁文件为 `0.4.0`，创建不可覆盖的 Linux 限定验收记录并提交；不推送。
- [ ] 只声明 Linux x64 / `g1_1` Remote SSH 限定支持。M12 OpenSSH 未选择、M13
  Windows x64 暂停、双平台 M14 最终声明继续保持 `待补测`。
...
当前 Linux 功能候选为
`docs/acceptance/2026-07-26-release-0.3.32-remote-editor-context.md`；在上述
